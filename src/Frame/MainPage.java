...
public class MainPage extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -589555884996294282L;
	private JPanel contentPane;
	Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
	public static int currentPlayerId;
	public static boolean online = false;
	public static boolean musicIsPlayed;
	public boolean singlePlayer;
	public boolean multyPlayer;
	...
	public MainPage() {
		if(!musicIsPlayed) {
			mp = new MusicPlayer("song/MainMenu.mp3", true);
			mp.start();
			musicIsPlayed = true;
		}
		...
		ImageIcon iconLogo = new ImageIcon(
				new ImageIcon("img/MainPage.png").getImage().getScaledInstance((int)screenSize.getWidth(), (int)screenSize.getHeight() , Image.SCALE_SMOOTH));
		contentPane.setLayout(null);
		...
		play.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if(!singlePlayer && !multyPlayer) {
					JOptionPane.showMessageDialog(null, "Please Select A Mode");
				}else if(singlePlayer) {
					mp.stop();
					musicIsPlayed = false;
					SinglePlayerSettingPage spsp = new SinglePlayerSettingPage();
					spsp.setVisible(true);
					dispose();
				} else if(multyPlayer) {
					mp.stop();
					musicIsPlayed = false;
					MultyPlayerSettingPage mpsp = new MultyPlayerSettingPage();
					mpsp.setVisible(true);
					dispose();
				}
			}
		});
		play.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		play.setBounds((int) (screenSize.getWidth()*51.3/100), (int) (screenSize.getHeight()*35.5/100), 200, 75);
		play.setBorder(null);
		contentPane.add(play);
		
		single = new JButton("single play");
		single.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new MusicPlayer("song/click.mp3", false).start();
				single.setBackground(new Color(51, 153, 255));
				singlePlayer = true;
				multy.setBackground(new Color(50, 50, 50));
				multyPlayer = false;
			}
		});
		single.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		single.setForeground(new Color(255, 204, 0));
		single.setBounds((int) (screenSize.getWidth()*37.6/100), (int) (screenSize.getHeight()*23.44/100), 155, 40);
		single.setBackground(new Color(50, 50, 50));
		single.setBorder(null);
		contentPane.add(single);
		
		multy = new JButton("multy player");
		multy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new MusicPlayer("song/click.mp3", false).start();
				if(online) {
					multy.setBackground(new Color(51, 153, 255));
					multyPlayer = true;
					single.setBackground(new Color(50, 50, 50));
					singlePlayer = false;
				}else {
		...
		multy.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		multy.setForeground(new Color(255, 204, 0));
		multy.setBounds((int) (screenSize.getWidth()*67.2/100), (int) (screenSize.getHeight()*23.44/100), 155, 40);
		multy.setBackground(new Color(50, 50, 50));
		multy.setBorder(null);
		contentPane.add(multy);
		
		JLabel profilePic = new JLabel();
		profilePic.setBounds(20, 27, 97, 97);
		contentPane.add(profilePic);
		ImageIcon pic = new ImageIcon(new ImageIcon(pq.players().get(currentPlayerId - 1).getPic()).getImage()
		...
		username.setText(pq.players().get(currentPlayerId - 1).getUserName());
		...
		level.setText(String.valueOf(pq.players().get(currentPlayerId - 1).getLevel()));
		...
		progressBar.setValue((int) (pq.players().get(currentPlayerId - 1).getXp() * 100)
				/ (pq.players().get(currentPlayerId - 1).getLevel() * 1000));
